    ...
    override fun addCompletions(
      ...
      if (parameters.position.prevSibling.elementType == QuantumTypes.DOLLAR) {
        for (param in scope.descendantsOfType<QuantumGlobalName>()) {
          ...
          result.addElement(
            LookupElementBuilder.create(param.text).withIcon(AllIcons.Nodes.Field).withTypeText("Global")
          )
        }
        ...
        if (scope.parent?.name == "scripts") {
          scope.parent?.parent?.parent?.subdirectories?.forEach {
            result.addElement(
              LookupElementBuilder.create(it.name).withIcon(AllIcons.Nodes.Package).withTypeText("Namespace")
            )
          }
        }
        ...
        for (param in listOf(
          ...
          if (parameters.position.text.startsWith(prefix)) {
            result.addElement(
              LookupElementBuilder.create(param).withIcon(AllIcons.Nodes.Type).withTypeText("Game Object")
            )
          }
        }
        for (param in listOf(
          ...
          if (parameters.position.text.startsWith(prefix)) {
            result.addElement(
              LookupElementBuilder.create(param).withIcon(AllIcons.Nodes.Class).withTypeText("Game Object")
            )
          }
        }
      } else {
        for (id in scope.descendantsOfType<QuantumParamName>().sortedBy { it.text }) {
          result.addElement(
            LookupElementBuilder.create("@${id.text}").withIcon(AllIcons.Nodes.Parameter)
              .withTypeText("Input Parameter")
          )
        }
